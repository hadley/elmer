...
(I also suspect that in general you'll get a mix of styles depending on what problem you apply the prompt to, so I'd recommend that you be explicit and ask for tidyverse code, if that's what you want.)
...
```{r}
instruct_json <- "
  Convert the following ingredients to nicely structured JSON.
  Just return the JSON.
"

chat <- chat_openai("You're an expert baker")
chat$chat(c(instruct_json, ingredients))
```
...
chat <- chat_openai("You're an expert baker")
chat$chat(c(instruct_json, instruct_weight, ingredients))
```
...
Now that I've iterated to get a data structure that I like, it seems useful to formalise it and tell the LLM exactly what I'm looking for using structured data. This guarantees that the LLM will only return JSON, the JSON will have the fields that you expect, and then elmer will automatically convert it into an R data structure for you. In this case, elmer recognises that an array of objects is equivalent to a data frame.
...
chat <- chat_openai("You're an expert baker")
chat$extract_data(
  c(instruct_json, instruct_weight, ingredients),
  spec = type_ingredients
)
```
