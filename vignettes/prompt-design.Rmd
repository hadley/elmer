...
There are a few logistics to discuss first. It's highly likely that you'll end up writing long, possibly multi-page prompts. So we have two recommendations to ensure that you're set up for success. First, put each prompt its own, separate file. Second, use markdown to write the prompts. The reason to use markdown is that it's quite readable to LLMs (and humans), and it allows you to do things like use headers to divide up a prompt into sections and itemised lists to enumerate multiple options. You can see some examples of this style of prompt here:
...
Unfortunately, you won't see these logistics in action in this vignette since we're keeping the prompts short and inline to make it easier for you to grok what's going on.
