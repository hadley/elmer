...
When I don't provide a system prompt, I get answers in a couple of languages:
...
Note that I've partioned my prompt in two pieces: the system prompt which defines the general behaviour and the user prompt which askes the specific question. You could put all of the content in the user prompt and get similar results, but I think it's helpful to split into two to divide the general framing of the response from the specific questions that you want to ask.
...
chat <- chat_openai(c(instruct_json, instruct_weight))
data <- chat$extract_data(ingredients, spec = type_ingredients)
str(data)
```
