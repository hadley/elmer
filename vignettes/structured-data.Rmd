...
chat <- chat_openai()
data <- chat$extract_data(text, type = wrapper)
...
type_disclosure_report <- type_object(
  assets = type_array(items = type_asset)
)

chat <- chat_openai()
image <- content_image_file("congressional-assets.png")
data <- chat$extract_data(image, type = type_disclosure_report)
str(data)
```
