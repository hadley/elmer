...
chat <- chat_openai()
str(chat$extract_data(prompt, spec = article_summary))
```
...
chat <- chat_openai()
str(chat$extract_data(prompt, spec = named_entities))
```
...
prompt <- interpolate("Extract structured data from <text>{{text}}</text>")
chat <- chat_openai()
str(chat$extract_data(prompt, spec = sentiment))
```
...
chat <- chat_openai()
chat$extract_data(prompt, spec = classification)
```
...
chat <- chat_claude()
chat$extract_data(prompt, spec = characteristics)
```
