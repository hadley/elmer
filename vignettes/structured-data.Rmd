...
To extract structured data you call `$extract_data()` instead of the `$chat()`. You'll also need to define a type specification that describes the structure of the data that you want (more on that shortly). Here's a simple example that extracts two few values from a string:
...
Now that you've seen a few examples, it's time to
